...
class CodeGenerator:
    ...
    def identifier(self, expr, precedence=None):
    ...
    def templateliteral(self, stmt, precedence):
        result = []
        for num, quasis in enumerate(stmt['quasis']):
            result.append(self.generate_expression(quasis, Precedence.Sequence))
            if num < len(stmt['expressions']):
                result.append("${%s}" % (self.generate_expression(stmt['expressions'][num], Precedence.Sequence)))
        return "`%s`" % (''.join(result))

    def templateelement(self, stmt, precedence):
        return str(stmt['value']['cooked'])
    ...
    def generate_arrow_function_body(self, node):
        result = [self.generate_function_params(node), self.space, '=>', self.space]
        
        if node["body"]["type"] == "ObjectExpression":
            result.extend(["(", self.generate_statement(node["body"]), ")"])
        else:
            result.extend([self.generate_statement(node["body"])])
